...
export default function Tags() {
  ...
  const loadTags = async () => {
    ...
    try {
      setLoading(true);
      const data = await tagService.getAll(token);
      setTags(data);
    } catch (err: any) {
  ...
  const save = async () => {
    ...
    try {
      if (editId) {
        await tagService.update(editId, form, token);
      } else {
        await tagService.create(form, token);
      }
      setShowModal(false);
      await loadTags();
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--muted-foreground)' }}>
            {tags.length} etiquetas cadastradas
          </p>
        </div>
        <button
          onClick={openNew}
      ...
      >
        ...
        <p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>
          Adicione etiquetas às transações para classificações cruzadas independentes de categorias.
          Ex: "Marido", "Presente", "Trabalho", "Viagem Europa". Filtre por etiquetas no extrato e
          gere relatórios por etiqueta.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <button
          onClick={openNew}
          className="rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all hover:opacity-70"
        ...
        >
          ...
          >
            ...
            <p className="text-sm mb-1" style={{ color: 'var(--muted-foreground)' }}>
              A etiqueta será removida de{' '}
              <strong>{tags.find((t) => t.id === confirmDelete)?.count || 0}</strong> transações
              associadas. As transações não serão excluídas.
            </p>
