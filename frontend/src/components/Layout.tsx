    ...
    <div className="flex min-h-screen" style={{ background: 'var(--background)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        >
          <div className="flex items-center gap-2" style={{ color: 'var(--muted-foreground)' }}>
            <span className="text-sm">{nav.find((n) => n.id === currentPage)?.label}</span>
          </div>
              ...
              {notifOpen && (
                ...
                >
                  ...
                  {[
                    {
                      icon: '⚠️',
                      text: 'Lazer: orçamento 140% comprometido',
                      time: 'Agora',
                      color: 'var(--danger)',
                    },
                    {
                      icon: '📅',
                      text: 'Fatura Cartão Inter vence em 2 dias',
                      time: '2h',
                      color: 'var(--warning)',
                    },
                    {
                      icon: '🎯',
                      text: 'Meta "Viagem Europa" está 56% completa!',
                      time: '1d',
                      color: 'var(--primary)',
                    },
                  ].map((n, i) => (
                    <div
                      key={i}
                      className="px-4 py-3 flex items-start gap-3 border-b transition-colors hover:bg-secondary/40 cursor-pointer"
                      style={{ borderColor: 'var(--border)' }}
                    >
                  ...
                  <button
                    className="w-full py-2.5 text-xs text-center transition-opacity hover:opacity-70"
                    style={{ color: 'var(--muted-foreground)' }}
                  >
                    Ver todas
                  </button>
      ...
      {showLockOverlay && (
        <div
          ...
          style={{ background: 'rgba(7,9,13,0.92)', backdropFilter: 'blur(8px)' }}
        >
          <div
            className="w-80 rounded-xl p-8 text-center"
            style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
          >
            <div
            ...
            <button
              onClick={unlock}
              className="w-full py-2.5 rounded-lg text-sm font-medium mt-2 transition-opacity hover:opacity-90"
              style={{ background: 'var(--primary)', color: '#fff' }}
            >
              Desbloquear
            </button>
